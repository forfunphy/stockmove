...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const fetchRemoteData = async () => {
      ...
      try {
        ...
        const firstCode = Object.keys(stockMap)[0];
        if (firstCode) {
          setSelectedStockCode(firstCode);
          // Need to defer this initialization slightly or just call it directly
          // We can't call initializeSimulation here easily because it depends on state that might not be set yet if we were using it from a callback, 
          // but here we are setting state.
          // However, initializeSimulation uses `config` which is state. It should be fine as `config` is initial state.
          // But we need to pass the data directly.

          // Logic from initializeSimulation duplicated/inlined to avoid stale state issues or dependency cycles
          const data = stockMap[firstCode];
          const targetDate = new Date(config.startYear, config.startMonth - 1, 1);
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      if (text) {
        try {
          // Simple CSV parser for uploaded files (simplified from original)
          ...
          Object.keys(stockMap).forEach(c => {
            stockMap[c] = calculateMovingAverages(stockMap[c].sort((a, b) => a.timestamp - b.timestamp));
          });
          ...
          const firstDate = new Date(stockMap[firstCode][0].date);
          const newConfig = {
            ...config,
            startYear: firstDate.getFullYear(), // Update year to match data
            startMonth: firstDate.getMonth() + 1
          };
          setConfig(newConfig);
          initializeSimulation(stockMap[firstCode], newConfig.startYear, newConfig.startMonth);
        } catch (error) {
          alert("讀取檔案失敗");
        }
      }
          ...
          <div className="w-full h-full bg-gray-800 rounded-lg shadow-inner overflow-hidden border border-gray-700 p-2">
            {isLoading ? (
              <div className="flex flex-col items-center justify-center h-full text-gray-500 animate-pulse gap-2">
                ...
                <div className="text-xs">正在從 GitHub Pages 讀取歷史數據...</div>
              </div>
            ...
            ) : <div className="flex items-center justify-center h-full text-gray-500">暫無數據</div>}
          </div>
        </div>
